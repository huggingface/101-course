...
**Tech Stack**: HTML/JavaScript (UI) + Inference Providers (AI)
...
When prompted, paste your Hugging Face token. Generate one from [your settings](https://huggingface.co/settings/tokens).

</hfoption>
<hfoption id="javascript">

You'll need your Hugging Face token. Get one from [your settings](https://huggingface.co/settings/tokens). Set it as an environment variable in your app.
...
When deploying to Spaces, add your token as a secret to avoid exposing it in code.
...
This uses Gradio's `gr.Audio` component for upload or microphone input, with two outputs: transcript and summary.
...
Use the `InferenceClient` from `@huggingface/inference` to call transcription and summarization.
...
Next, use a widely available language model like `deepseek-ai/DeepSeek-V3-0324` via an Inference Provider. Use `provider="auto"` to select an available provider.
...
Next, use a language model like `deepseek-ai/DeepSeek-V3-0324` via an Inference Provider, with `provider: "auto"`.
We will define a custom prompt to ensure the output is formatted as a summary with action items and decisions made:
