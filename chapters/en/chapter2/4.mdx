...
Inference Endpoints let you deploy a specific model from the Hub as a dedicated, production-grade API with autoscaling, security, and configurability.

In this unit, you'll deploy an endpoint via the UI, learn where to find its URL and token requirements, and see minimal client code to interact with it.

## What you get

- Dedicated, isolated serving for a single model
- Choice of cloud vendor, region, and accelerator
- Instance size/type selection and autoscaling
...
for ep in list_inference_endpoints():
    print(ep.name, ep.status)

ep = get_inference_endpoint("my-endpoint-name")
print(ep.url)
```
...
endpoint = create_inference_endpoint(
    ...
    custom_image={
        ...
        "url": "ghcr.io/my-custom-image:latest",
    },
)
```
...
Allow a few minutes for a new endpoint to become ready. Check status with `get_inference_endpoint(name)` and only send traffic once it's `READY`.

</Tip>


Up next: build a small app that targets your endpoint and adds production tips.
